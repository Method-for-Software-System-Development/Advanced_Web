...
const PetCard: React.FC<{ pet: Pet }> = ({ pet }) => {
  ...
  return (
    <li className="w-full mb-4 bg-[var(--color-cream)] dark:bg-[#4A2F33] rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-6 flex flex-col min-h-[120px] relative text-base text-[var(--color-greyText)] dark:text-gray-200 border border-gray-200 dark:border-gray-600">      <div className="flex items-center justify-between mb-4">        <div className="flex items-center gap-3">
          {/* Pet image next to name */}          <div className="relative">
            {currentPet.imageUrl ? (
              <img 
                src={getImageUrl(currentPet.imageUrl)} 
                alt={`${currentPet.name} - ${currentPet.type}`}
                className="w-22 h-22 object-cover rounded-full shadow-md border-2 border-[var(--color-skyDark)] dark:border-[#4A7C7D]"
              />            ) : (
