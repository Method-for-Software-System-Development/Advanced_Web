...
/**
 * Displays a list of pets for the client and allows selection of a pet (radio buttons).
 * Optionally displays pet type, breed, birth year, and weight.
 *
 * Props:
 * - pets: Array of Pet objects to display
 * - selectedPetId: The currently selected pet's ID
 * - onPetSelect: Callback when a pet is selected (petId: string)
 * - clientName: The client's name (for empty state message)
 * - showPetType: Whether to show type/breed/birthYear/weight (default: true)
 */
...
interface PetSelectionClientProps {
  ...
  /** Callback when a pet is selected */
  onPetSelect: (petId: string) => void;
  /** Client's name for empty state message */
  clientName: string;
  showPetType?: boolean;
}
...
const PetSelectionClient: React.FC<PetSelectionClientProps> = ({
  ...
  // If no pets, show a message to add a pet
  if (pets.length === 0) {
    return (
  ...
  // Render the pet selection radio group
  return (
    <div className="border-b pb-4 dark:border-gray-600">
      <h3 className="text-lg font-semibold text-gray-700 mb-3 dark:text-[#FDF6F0]">Pet Information</h3>
      <div>
        ...
        <div className="space-y-2">
          {pets.map(pet => (
            // Each pet is a radio button option
            <label key={pet._id} className="flex items-center space-x-2 p-2 border border-gray-200 rounded-md hover:bg-gray-50 cursor-pointer dark:border-gray-600 dark:hover:bg-gray-700">
              ...
              <div className="flex-1">
                ...
                {/* Show type/breed/birthYear/weight if showPetType is true */}
                {showPetType && (
                  <>
